function qc_eeg(folderData, filenameDataCommon,nTetrode)

% for each tetrode, there are expected to be 4 channels, so nTetrode*4
% channels in total

% load input data (i.e. timestamps of shock/tone
inputs = LoadData(sprintf('%s/%s',folderData, filenameDataCommon), nTetrode);
...
for iChannel = 1%:(nTetrode*4)
    switch iChannel
        ...
        otherwise
            eeg_filename = sprintf('%s/%s.eeg%i', folderData, filenameDataCommon,iChannel);
    end
    % load eeg data
    ...
    for iShock = 1:5
        ...
        patch([xLeftEdge xRightEdge xRightEdge xLeftEdge], [yStart yStart yEnd yEnd], colorFilter, 'FaceAlpha',.1,'EdgeColor',colorFilter);
